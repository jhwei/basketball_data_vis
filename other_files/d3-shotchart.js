(function(global, factory) {
  ...
  function shots() {
    var hexRadiusValues = [.8, 1, 1.2], hexMinShotThreshold = 1,
      ...
      selection.each(function(data) {
        ...
        if (activeDisplay === "scatter") {
          ...
          for (var i = 0, l = hexBinCoords.length; i < l; i++) {
            var pts = hexBinCoords[i];
            var numPts = 0;

            for (var j = 0, jl = pts.length; j < jl; ++j) {
              numPts += pts[j].value.attempts || 1;
            }
            if (numPts > 1) hexbin_list.push(numPts);
          }

          hexRadiusScale =
              d3.scaleQuantize().domain(hexbin_list).range(hexRadiusValues);
          var shots =
